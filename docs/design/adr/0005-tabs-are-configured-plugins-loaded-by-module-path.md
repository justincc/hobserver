...
- **Other people adding tabs** without forking this repo. The intent is that
  someone can install hermes-observer, install or write their own tab —
...
- **`module` is any importable path.** `plugins.timing` (in this tree) and
  `hermes_observer_zep` (installed from elsewhere) load by exactly the same
...
hermes-observer package or track its version. The one real coupling is
