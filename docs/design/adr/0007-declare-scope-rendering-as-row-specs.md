...
that someone should be able to install hobserver, install or write their
...
**`hobserver.toml` stays what ADR 5 made it: tab wiring.** This is the
...
settings = { scope_specs = ["hobserver_acme.specs"] }
```
...
module path, and `plugins.turns.specs` and `hobserver_acme.specs`
